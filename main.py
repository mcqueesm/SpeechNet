...
def log_spectrogram(wav):
    freqs, times, spec = stft(wav, 16000, nperseg = 400, noverlap = 240, nfft = 512, 
...
if __name__ == '__main__':
    ...
    our_model = load_model('joni_model_sig.h5')
    

    count = 1
    try:
        while True:
            print("Say a Joni sign" + str(count))
            ...
            wavfile.write('/Users/jonirecord/goat' + str(count) + '.wav', RATE, data)

            count += 1
            
            
            #arr = our_model.predict(np.expand_dims(r, axis=0))[0]
            #pred = np.argmax(arr)
            #score = arr[pred]
            '''
            print("Prediction:")
